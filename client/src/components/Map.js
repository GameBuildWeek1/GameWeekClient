...
var renderFrame = () => {
  ...
  try {
    if (mapTemp[loc.y][loc.x] == 0) throw "";
    player.x = loc.x;
    player.y = loc.y;
  } catch {}
  ...
  players.forEach(p => {
    if (p.z == player.z) {
      if (!playerTexture) {
        ...
        ctxcam.fillRect(
          (p.x - camerpos.x) * tileSize,
          (p.y - camerpos.y) * tileSize,
          tileSize,
          tileSize
        );
      } else {
        ...
        ctxcam.drawImage(
          playerTexture,
          (p.x - camerpos.x) * tileSize - ((Math.round(a / 3) % 10) + 5),
          (p.y - camerpos.y) * tileSize - (Math.round(a / 15) % 10),
          tileSize,
          tileSize
        );
      }
      ...
      ctxcam.fillText(
        p.name,
        (p.x - camerpos.x) * tileSize + tileSize / 2 + 3,
        (p.y - camerpos.y) * tileSize - tileSize / 4 + 3
      );
      ctxcam.fillStyle = "#FFFFFFAA";
      ctxcam.fillText(
        p.name,
        (p.x - camerpos.x) * tileSize + tileSize / 2,
        (p.y - camerpos.y) * tileSize - tileSize / 4
      );
    }
  });
  if (!playerTexture) {
    ...
    ctxcam.fillRect(
      (player.x - camerpos.x) * tileSize,
      (player.y - camerpos.y) * tileSize,
      tileSize,
      tileSize
    );
  } else {
    ctxcam.drawImage(
      playerTexture,
      (player.x - camerpos.x) * tileSize /* -Math.random()*20+10 */,
      (player.y - camerpos.y) * tileSize /* -Math.random()*10 */,
      tileSize,
      tileSize
    );
  }
  ...
  ctxcam.fillText(
    "YOU",
    (player.x - camerpos.x) * tileSize + tileSize / 2 + 3,
    (player.y - camerpos.y) * tileSize - tileSize / 10 + 3
  );
  ctxcam.fillStyle = "#FFFFFFAA";
  ctxcam.fillText(
    "YOU",
    (player.x - camerpos.x) * tileSize + tileSize / 2,
    (player.y - camerpos.y) * tileSize - tileSize / 10
  );
  if (message) {
    ctxcam.font = "bold 15px Arial";
    ctxcam.textAlign = "center";
  ...
  for (var y = sy; y < mapTemp.length; y++) {
    try {
      for (var x = sx; x < mapTemp[y].length; x++) {
        ...
        } else if (mapTemp[y][x] == "E") ctxmap.fillStyle = "#CCA";
        else if (mapTemp[y][x] == "S") ctxmap.fillStyle = "#AAA";
        else {
        ...
        if (!texture)
          ctxmap.fillRect(
            x * tileSize - sx * tileSize,
            y * tileSize - sy * tileSize,
            tileSize - 1,
            tileSize - 1
          );
        else
...
const updateplayers = data => {
  ...
  if (data.map) mapTemp = data.map.data;
  if (!message) {
